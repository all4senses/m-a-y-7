	...
	<nav class="cd-primary-nav">
      ...
      <div class="navbar navbar-default yamm">
          <div class="navbar-header">
            ...
            <!-- <a href="#" class="navbar-brand">Upper menu</a> -->
          </div>
          <div id="navbar-collapse-2" class="navbar-collapse collapse">
              <?php 
              $menu = menu_tree_all_data('menu-upper-menu');
              echo get_menu_children_themed($menu);
...
<?php
  ...
  $menu_tree = get_menu_children_themed($menu);
...
//  }
  ...
  function get_menu_children_themed($menu_parent, $level = 0) {
    ...
    foreach ($menu_parent as $key => $item) {
      ...
      $layout .= '<li' . $li_class . '><a' . $a_class . ' href="' . ($item['link']['link_path'] == '<nolink>' ? '#' : $item['link']['link_path']) . '">' . $item['link']['link_title'] . $caret . '</a>';
      if (!empty($item['below'])) {
        $layout .= get_menu_children_themed($item['below'], $level + 1);
      }
      $layout .= '</li>';
    }
    $layout .= '</ul>';
  ...
  function get_menu_children($menu_parent) {
    $menu_current = array();
    $count = 0;
    foreach ($menu_parent as $key => $item) {
      $menu_current[$count]['title'] = $item['link']['link_title'];
      $menu_current[$count]['link'] = $item['link']['link_path'];
      if (!empty($item['below'])) {
        $menu_current[$count]['below'] = get_menu_children_themed($item['below']);
      }
      $count++;
    }
    return $menu_current;
...
<?php //print 'in themes page.tpl.php'; ?>    
